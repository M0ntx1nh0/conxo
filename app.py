...
def _draw_metric_card_pdf(
    ...
    value_font = _load_pdf_font(32, bold=True)
    label_bbox = draw.textbbox((0, 0), label, font=label_font)
    value_bbox = draw.textbbox((0, 0), value, font=value_font)
    draw.text(
        (x1 + (x2 - x1 - (label_bbox[2] - label_bbox[0])) / 2, y1 + 24),
        label,
        font=label_font,
        fill="#6b7c8f",
    )
    draw.text(
        (x1 + (x2 - x1 - (value_bbox[2] - value_bbox[0])) / 2, y1 + 56),
        value,
        font=value_font,
        fill=value_fill,
    )
...
def build_player_report_pdf(
    ...
    body_font = _load_pdf_font(30, bold=False)
    small_font = _load_pdf_font(24, bold=False)
    ...
    section_title = _load_pdf_font(44, bold=True)
    h1 = _load_pdf_font(34, bold=True)
    h2 = _load_pdf_font(24, bold=True)
    value_font_big = _load_pdf_font(34, bold=True)
    body_small = _load_pdf_font(22, bold=False)

    ddraw.rounded_rectangle((54, 42, 1700, 170), radius=36, fill="#10364d")
    ddraw.text((90, 78), player_name.upper(), font=section_title, fill="#ffffff")
    ...
    for label, value in bio_pairs:
        ...
        ddraw.text((86, bio_y + 36), value, font=value_font_big, fill="#10364d")
        bio_y += bio_step

    left_col_x = 86
    right_col_x = 332
    ddraw.text((left_col_x, bio_y), "Año de nacimiento", font=h2, fill="#6b7c8f")
    ddraw.text((left_col_x, bio_y + 36), fecha_nacimiento, font=value_font_big, fill="#10364d")
    ddraw.text((right_col_x, bio_y), "Edad", font=h2, fill="#6b7c8f")
    ddraw.text((right_col_x, bio_y + 36), edad_value, font=value_font_big, fill="#10364d")
    ...
    _draw_lines(ddraw, (86, text_start_y), second_page_lines, comments_font, "#10364d", line_gap=6)
